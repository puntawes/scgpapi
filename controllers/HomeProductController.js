...
const { getPageContent, old_attachment_link,get_url_rewrite, url_link,newbyf_attachment_link } = require("./Controllers");
    ...
    const get_product_category_attachment =  async (main_id, lang, content_thumbnail,content_subject,content_rewrite_id ) => {
        ...
        if(data_all[0]['attachment_base'] === content_thumbnail) {
            ...
            //data_all[0]['thumbnail_link'] = await newbyf_attachment_link('images', 'product_category', data_all[0]['attachment_base']);
        }
    ...
    exports.index = async (req, res, next) => {
        try {
            ...
            if (getContent['product'] !== "") {
                for (const [key, value] of Object.entries(getContent['product'])) {
                    var get_product_category_contents = await get_product_category_content(lang, value['main_id'], value['content_id'], 'product');
